...
#' Construct scenario list of gcm, ssp, rcp combinations
zip_scenario <- function (gcm_vector, ssp_vector, rcp_vector, infeasible) {
    
    v <- c()
    ix <- 1
    for (i in 1:length(gcm_vector)) {
        gcm <- gcm_vector[[i]]
        
        for (j in 1:length(ssp_vector)) {
            ssp <- ssp_vector[[j]]
            
            for (k in 1:length(rcp_vector)) {
                rcp <- rcp_vector[[k]]
                
                # construct key to conduct lookup
                key <- paste0(ssp, "-", rcp, "-", gcm)

                # if the target value is not in the infeasible scenarios add it to list
                if (!key %in% infeasible) {
                    v[[ix]] <- list(gcm=gcm, ssp=ssp, rcp=rcp)
                    ix <- ix + 1  
                }
            }
        }
...
#' Adjust demeter urban area and other landclasses using SELECT urban extent
adjust_demeter <- function(yr, select_input_dir, demeter_input_dir, demeter_base_file, demeter_output_dir, 
                           output_demeter_base_raster, method, method_path, scenario) {
    
    print(paste0("Processing year:  ", yr))
    
    # construct demeter netcdf basename based on the scenario
    demeter_file_name <- paste0("LU", method, "_", scenario$ssp, "_",  scenario$rcp, "_", scenario$gcm, "_", yr, ".nc")
    demeter_file_path <- file.path(demeter_input_dir, method_path, demeter_file_name)
    
    # construct SELECT file name
    select_file_name <- paste0("Select_", scenario$ssp, "_", yr, "_sum.tif")
...
method_path_vector <- c('unharmonized_netcdf', 'harmonized_netcdf')
method_vector <- c('', '_harmonized')
gcm_vector <- c('gfdl', 'hadgem', 'ipsl', 'miroc', 'noresm')
ssp_vector <- c('ssp1', 'ssp2', 'ssp3', 'ssp4', 'ssp5')
rcp_vector <- c('rcp26', 'rcp45', 'rcp60', 'rcp85')
yr_vector <- seq(start_yr, through_yr, time_step)

# list of infeasbile GCAM runs from Graham et al.  There are no demeter runs to accomodate these scenarios
infeasible <- c('ssp1-rcp85-gfdl', 'ssp1-rcp85-ipsl', 'ssp1-rcp85-miroc', 'ssp1-rcp85-noresm', 'ssp1-rcp85-hadgem',
                'ssp2-rcp85-gfdl', 'ssp2-rcp85-ipsl', 'ssp2-rcp85-miroc', 'ssp2-rcp85-noresm', 'ssp2-rcp85-hadgem',
                'ssp3-rcp26-gfdl', 'ssp3-rcp26-ipsl', 'ssp3-rcp26-miroc', 'ssp3-rcp26-noresm', 'ssp3-rcp26-hadgem',
                'ssp3-rcp85-gfdl', 'ssp3-rcp85-ipsl', 'ssp3-rcp85-miroc', 'ssp3-rcp85-noresm', 'ssp3-rcp85-hadgem',
                'ssp4-rcp85-gfdl', 'ssp4-rcp85-ipsl', 'ssp4-rcp85-miroc', 'ssp4-rcp85-noresm', 'ssp4-rcp85-hadgem')

method <- method_vector[[method_idx]]
method_path <- method_path_vector[[method_idx]]
scenario <- zip_scenario(gcm_vector, ssp_vector, rcp_vector, infeasible)[[run_idx]]

print(paste0("Processing scenario:  ", method_path, "-", scenario$gcm, "-", scenario$ssp, "-", scenario$rcp))
...
#foreach::foreach(i = 1:length(yr_vector),
#                 .packages=c('raster', 'ncdf4', 'maptools', 'maps', 'rgdal', 'dplyr', 'tools', 'tibble')
#                 ) %dopar% {

for (yr in yr_vector) {    

    adjust_demeter(yr, select_input_dir, demeter_input_dir, demeter_base_file, demeter_output_dir, 
                   output_demeter_base_raster, method, method_path, scenario)
