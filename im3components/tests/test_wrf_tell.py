...
class TestWrfTell(unittest.TestCase):
    ...
    def test_future_balancing_authority_aggregation(self):
        """Ensure that historical counties to BAs aggregation produces the same result."""
        ...
        # remove output file if it exists
        try:
            os.remove(f'{self.data_path}/PSEI_WRF_Hourly_Mean_Meteorology_2059.csv')
        except OSError:
            pass
        ...
        wrf_to_tell_balancing_authorities(
            ...
            is_historical=False,
            balancing_authority_to_fips_file=f'{self.data_path}/fips_service_match_2019.csv',
            county_population_by_year_file=f'{self.data_path}/ssp3_county_population.csv',
            county_data_directory=f'{self.data_path}',
            output_directory=f'{self.data_path}',
            output_file_infix='WRF_Hourly_Mean_Meteorology',
            county_data_prefix='',
        ...
        # load the output and check results
        data = pd.read_csv(f'{self.data_path}/PSEI_WRF_Hourly_Mean_Meteorology_2059.csv')

        validation_data = pd.DataFrame({
            'Time_UTC': np.array(['2059-01-01 01:00:00', '2059-01-01 02:00:00']),
            'T2': np.array([270.14, 269.46], dtype=float),
            'Q2': np.array([0.00242, 0.0023], dtype=float),
            'SWDOWN': np.array([0.0, 0.0], dtype=float),
            'GLW': np.array([218.26, 217.44], dtype=float),
            'WSPD': np.array([1.92, 2.33], dtype=float),
        })
